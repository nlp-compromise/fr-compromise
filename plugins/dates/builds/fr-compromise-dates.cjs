(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.frCompromiseDates = factory());
  ...
  const parseNumber$2 = function (s, input) {
  ...
  const parseMonth$1 = function (str) {
    str = str.toLowerCase().trim();
    if (str === 'sept') {
      return months$1.sep
    }
  ...
  const { parseArray, parseObject, parseNumber: parseNumber$1 } = fns;
  ...
  */
  ...
  const parseMonth = function (m) {
    ...
    if (months$2.hasOwnProperty(str)) {
      return months$2[str] - 1
    }
    return null
  };
  ...
  const startEnd = function (m, opts) {
    if (m.found) {
      ...
      let out = {
        start: parseOne$1(start, opts),
        end: parseOne$1(end, opts)
      };
      if (out.start) {
        return out
      }
  ...
  const justStart = function (m, opts) {
    let out = { start: parseOne$1(m, opts) };
    if (out.start) {
      return out
    }
    return null
  ...
  const untilEnd = function (m, opts) {
    let { end } = m.groups();
    let out = { start: new Moment(opts.today, opts), end: parseOne$1(end, opts) };
    if (out.end) {
      // until - just before x
  ...
  const api = function (View) {
    class Dates extends View {
      ...
      parse(n) {
        return getNth(this, n).map(m => toJson$1(parse(m.this.opts)))
      }
      json(opts, n) {
        let m = getNth(this, n);
        let arr = m.map(vb => {
